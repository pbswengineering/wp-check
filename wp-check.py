...
class WordPressOnline:
	...
	def get_wp_branch_last_version_release_date(self, wp_version: str) -> Optional[datetime]:
		"""
	...
	def get_plugin_last_version(self, plugin_slug: str) -> Tuple[PackageVersion, bool]:
		"""
...
def is_wordpress(directory: str) -> bool:
	...
	"""
	return os.path.exists(os.path.join(directory, "wp-content")) and os.path.exists(os.path.join(directory, "wp-includes"))
...
def check_plugin(wp: WordPressOnline, plugins_dir: str, plugin_slug: str):
	...
	if closed:
		...
		print(f"   {plugin_slug}: updated required from {plugin_version} to {last_version}")
